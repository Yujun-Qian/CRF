...
def set_work_dir(local_path="ner_bert_crf", server_path="ner_bert_crf"):
    if (os.path.exists(("/home/share_oversea/qianyujun/")+'/'+local_path)):
        os.chdir(("/home/share_oversea/qianyujun/")+'/'+local_path)
    elif (os.path.exists(("/home/share_oversea/qianyujun/")+'/'+server_path)):
        os.chdir(("/home/share_oversea/qianyujun/")+'/'+server_path)
    else:
...
def get_data_dir(local_path="ner_bert_crf", server_path="ner_bert_crf"):
    if (os.path.exists(("/home/share_oversea/qianyujun/")+'/'+local_path)):
        return ("/home/share_oversea/qianyujun/")+'/'+local_path
    elif (os.path.exists(("/home/share_oversea/qianyujun/")+'/'+server_path)):
        return ("/home/share_oversea/qianyujun/")+'/'+server_path
    else:
...
class InputFeatures(object):
    ...
    def __init__(self, input_ids, input_mask, segment_ids,  predict_mask, labels):
...
class NerDataset(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return feat.input_ids, feat.input_mask, feat.segment_ids, feat.predict_mask, feat.labels
...
#    print(f"guid: {example.guid}, words: {example.words}, labels: {example.labels}")
...
os.environ['HF_HOME'] = '../models--bert-base-cased/'
tokenizer = AutoTokenizer.from_pretrained('../models--bert-base-cased/models--bert-base-cased/snapshots/cd5ef92a9fb2f889e972770a36d4ed042daf221e/')
...
def evaluate(model, predict_dataloader, batch_size, epoch_th, dataset_name):
    ...
    with torch.no_grad():
        for batch in predict_dataloader:
            ...
            out_scores = out_scores.logits
            _, predicted = torch.max(out_scores, -1)
            valid_predicted = torch.masked_select(predicted, predict_mask.bool())
            valid_label_ids = torch.masked_select(label_ids, predict_mask.bool())
...
class BERT_CRF_NER(nn.Module):
    ...
    def _get_bert_features(self, input_ids, segment_ids, input_mask):
        ...
        bert_seq_out = o.last_hidden_state
        #print(bert_seq_out)
...
bert_model = BertModel.from_pretrained('../models--bert-base-cased/models--bert-base-cased/snapshots/cd5ef92a9fb2f889e972770a36d4ed042daf221e/')
...
if load_checkpoint and os.path.exists(output_dir+'/ner_bert_crf_checkpoint.pt'):
    checkpoint = torch.load(output_dir+'/ner_bert_crf_checkpoint.pt', map_location='cpu', weights_only=False)
...
#optimizer = AdamW(model.parameters(), lr=learning_rate0)
optimizer = optim.Adam(model.parameters(), lr=learning_rate0)
scheduler = get_linear_schedule_with_warmup(optimizer, num_warmup_steps=warmup_proportion * total_train_steps, num_training_steps=total_train_steps)
...
for epoch in range(start_epoch, total_train_epochs):
    print(f"start_epoch: {start_epoch}, total_train_epochs: {total_train_epochs}, epoch: {epoch}")
    tr_loss = 0
    train_start = time.time()
    model.train()
    optimizer.zero_grad()
    ...
    for step, batch in enumerate(train_dataloader):
        batch1 = tuple(t.to(device) for t in batch)
        input_ids, input_mask, segment_ids, predict_mask, label_ids = batch1
        ...
        if (step + 1) % gradient_accumulation_steps == 0:
            ...
            print(f"lr_this_step: {lr_this_step}, global_step_th/total_train_steps: {global_step_th/total_train_steps}, warmup_proportion:{warmup_proportion}")
            for param_group in optimizer.param_groups:
                param_group['lr'] = lr_this_step
            optimizer.step()
            optimizer.zero_grad()
            global_step_th += 1
            print("Epoch:{}-{}/{}, Negative loglikelihood: {} ".format(epoch, step, len(train_dataloader), neg_log_likelihood.item()), flush=True)
            del neg_log_likelihood
            #torch.save({'epoch': epoch, 'model_state': model.state_dict(),
            #    'max_seq_length': max_seq_length, 'lower_case': do_lower_case},
            #        os.path.join(output_dir, 'ner_bert_crf_checkpoint.pt.test'))
            #valid_acc, valid_f1 = evaluate(model, dev_dataloader, batch_size, epoch, 'Valid_set')
        gc.collect()
        del batch

    print('--------------------------------------------------------------')
    #print("Epoch:{} completed, Total training's Loss: {}, Spend: {}m".format(epoch, tr_loss, (time.time() - train_start)/60.0), flush=True)
    print("Epoch:{} completed, Total training's Loss:, Spend: {}m".format(epoch, (time.time() - train_start)/60.0), flush=True)
    valid_acc, valid_f1 = evaluate(model, dev_dataloader, batch_size, epoch, 'Valid_set')
